...
def newDeleteJob(chat_id, message_id, delete_command = False, command_id = None, delete_delay = 300):
        ...
        deleteList.append({
            ...
            "time": datetime.utcnow() + timedelta(seconds = int(delete_delay))  
        })
    ...
    for job in deleteList:
        ...
        if job["time"] < datetime.utcnow():
            ...
            if job["delete_command"] and job["command_id"]:
                ...
                try:
                    await client.delete_messages(chat_id = job["chat_id"], message_ids = job["command_id"])
