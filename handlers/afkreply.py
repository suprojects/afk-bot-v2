        ...
        try:
            ...
            if member.status == 'creator' or member.status == 'administrator' or member.status == 'member':
                ...
                if status.get('afk_media', False) and group_settings.get('afk_media', True):
                    ...
                    if status['afk_media']['type'] == 'video': x = await m.reply_video(video = status['afk_media']['id'], caption = reply_message, parse_mode = 'markdown')
                    elif status['afk_media']['type'] == 'photo': x = await m.reply_photo(photo = status['afk_media']['id'], caption = reply_message, parse_mode = 'markdown')
                    else: x = await m.reply(reply_message, parse_mode = 'markdown', disable_web_page_preview = True)
